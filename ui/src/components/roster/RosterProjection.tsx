...
export function RosterProjection({ roster, rosterConfig }: Props) {
  const rankingMap = useMemo(() => {
    const map = new Map<string, { projection: number; floor: number; ceiling: number }>()
    for (const r of MOCK_RANKINGS) {
      map.set(r.player.id, { projection: r.projection, floor: r.floor, ceiling: r.ceiling })
    }
    return map
  }, [])
  ...
  const mean = starters.reduce((s, p) => s + (rankingMap.get(p.id)?.projection ?? p.projection.mean), 0)
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="space-y-1">
        ...
        {starters.map(p => {
          const proj = rankingMap.get(p.id)?.projection ?? p.projection.mean
          return (
            <div key={p.id} className="flex items-center gap-2 bg-bg-elevated rounded-lg px-3 py-2">
              ...
              <span className="text-xs text-text-secondary tabular-nums">{proj.toFixed(1)} pts</span>
            </div>
          )
        })}
      </div>
